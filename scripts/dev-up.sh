...
CONNECTOR_BACKPRESSURE_DRAFTS_LOG="$LOG_DIR/connector-backpressure-drafts.log"
CONNECTOR_GUARDIAN_LOG="$LOG_DIR/connector-guardian.log"
...
CONNECTOR_BACKPRESSURE_DRAFTS_ENABLED="$(read_env_value FLOWSTATE_ENABLE_CONNECTOR_BACKPRESSURE_DRAFTS || true)"
CONNECTOR_BACKPRESSURE_DRAFTS_ENABLED_NORMALIZED="$(printf '%s' "$CONNECTOR_BACKPRESSURE_DRAFTS_ENABLED" | tr '[:upper:]' '[:lower:]')"
CONNECTOR_GUARDIAN_ENABLED="$(read_env_value FLOWSTATE_ENABLE_CONNECTOR_GUARDIAN || true)"
CONNECTOR_GUARDIAN_ENABLED_NORMALIZED="$(printf '%s' "$CONNECTOR_GUARDIAN_ENABLED" | tr '[:upper:]' '[:lower:]')"
...
if [[ "$CONNECTOR_BACKPRESSURE_DRAFTS_ENABLED_NORMALIZED" == "1" || "$CONNECTOR_BACKPRESSURE_DRAFTS_ENABLED_NORMALIZED" == "true" || "$CONNECTOR_BACKPRESSURE_DRAFTS_ENABLED_NORMALIZED" == "yes" ]]; then
  start_service \
    "connector-backpressure-drafts" \
    "$CONNECTOR_BACKPRESSURE_DRAFTS_PID_FILE" \
    "$CONNECTOR_BACKPRESSURE_DRAFTS_LOG" \
    env FLOWSTATE_EFFECTIVE_OPENAI_API_KEY="$OPENAI_API_KEY_VALUE" bash -lc "cd '$ROOT_DIR' && set -a && source '$ENV_FILE' && set +a && export OPENAI_API_KEY=\"\$FLOWSTATE_EFFECTIVE_OPENAI_API_KEY\" && npm run watch:connector-backpressure-drafts --workspace @flowstate/worker"
fi

if [[ "$CONNECTOR_GUARDIAN_ENABLED_NORMALIZED" == "1" || "$CONNECTOR_GUARDIAN_ENABLED_NORMALIZED" == "true" || "$CONNECTOR_GUARDIAN_ENABLED_NORMALIZED" == "yes" ]]; then
  start_service \
    "connector-guardian" \
    "$CONNECTOR_GUARDIAN_PID_FILE" \
    "$CONNECTOR_GUARDIAN_LOG" \
    env FLOWSTATE_EFFECTIVE_OPENAI_API_KEY="$OPENAI_API_KEY_VALUE" bash -lc "cd '$ROOT_DIR' && set -a && source '$ENV_FILE' && set +a && export OPENAI_API_KEY=\"\$FLOWSTATE_EFFECTIVE_OPENAI_API_KEY\" && npm run watch:connector-guardian --workspace @flowstate/worker"
fi
